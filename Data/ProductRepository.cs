    ...
    public class ProductRepository
    {
        ...
        public async Task<string> AddProduct(ProductModel product)
        {
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                cmd.Parameters.AddWithValue("@ProductImage", string.IsNullOrWhiteSpace(product.ProductImage) ? (object)DBNull.Value : product.ProductImage);
                cmd.Parameters.AddWithValue("@ProductBrandId", product.ProductBrandId);
                var priceParam = cmd.Parameters.Add("@ProductPrice", SqlDbType.Decimal);
                priceParam.Precision = 18;
                priceParam.Scale = 2;
                priceParam.Value = product.ProductPrice;
